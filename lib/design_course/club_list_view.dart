  ...
  Widget _buildBody(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot<QuerySnapshot<Map<String, dynamic>>> snapshot){
        ...
        return _buildList(context, groupDocs);
      },
    );
  }

  Widget _buildList(BuildContext context, List<QueryDocumentSnapshot<Map<String, dynamic>>> groupDocs) {
    List<QueryDocumentSnapshot<Map<String, dynamic>>> searchResults = [];
    if(widget.filterData.length == 0) {
      for(QueryDocumentSnapshot<Map<String, dynamic>> d in groupDocs) {
      ...
      for(int i = 0; i < widget.filterData.length; i++) {
        for(QueryDocumentSnapshot<Map<String, dynamic>> d in groupDocs) {
          if (d.get('category') == widget.filterData[i]) {
            searchResults.add(d);
          }
